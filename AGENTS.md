...
Existing ids: `tool-use`, `rag-vs-mcp` (retrieve vs tool), `appsec-prompt`, `appsec-withhold` (secret withhold).
...
- Do not score from agent prose alone (`rag-vs-mcp` needs decision JSON; `appsec-prompt` needs `path:line`; `appsec-withhold` needs withhold JSON).
